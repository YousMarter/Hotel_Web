...
public class ResetService extends HttpServlet implements java.io.Serializable {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			try {
				...
				ui.settemppassword(createpassword());
				ui.setpassword(createpassword());
				System.out.println(ui.password);
				writepassword();
				SendMail mail=new SendMail();
				mail.send(0, ui);
				
				// if ui got// todo: send a reset email.
			...
			try {
				...
				out.print("<script>alert('No email found, Make sure you input the registered email.'); window.location='reset.jsp' </script>");
				out.flush();
				out.close();
		//		response.sendRedirect("reset.jsp");
			} catch (IOException e) {
